import { ReactNode } from 'react'
import { Phone } from 'lucide-react'

import {
  Avatar,
  AvatarFallback,
  AvatarImage,
} from 'components/ui/avatar'
import { Button } from 'components/ui/button'
import { cn } from 'components/lib/utils'

import { BacentaWithArrivals } from '../arrivals-types'

type BacentaArrivalsCardProps = {
  bacenta: BacentaWithArrivals
  onClick?: () => void
  contact?: boolean
  children?: ReactNode
  className?: string
}

const getInitials = (name: string) => {
  const trimmed = name?.trim() ?? ''
  if (!trimmed) return '··'
  const parts = trimmed.split(/\s+/)
  if (parts.length === 1) return parts[0].slice(0, 2).toUpperCase()
  return `${parts[0][0] ?? ''}${parts[parts.length - 1][0] ?? ''}`.toUpperCase()
}

const BacentaArrivalsCard = ({
  bacenta,
  onClick,
  contact = true,
  children,
  className,
}: BacentaArrivalsCardProps) => {
  const leader = bacenta.leader
  const leaderName = leader?.nameWithTitle ?? leader?.fullName ?? ''
  const initials = getInitials(bacenta.name ?? '')
  const showContact =
    contact && !!(leader?.phoneNumber || leader?.whatsappNumber)

  const Body = onClick ? 'button' : 'div'

  return (
    <div
      className={cn(
        'flex flex-col rounded-xl border border-border bg-card overflow-hidden',
        className
      )}
    >
      <Body
        type={onClick ? 'button' : undefined}
        onClick={onClick}
        className={cn(
          'flex flex-1 flex-col items-center gap-3 p-4 text-center w-full',
          onClick &&
            'cursor-pointer transition-colors hover:bg-muted/50 active:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring'
        )}
      >
        <Avatar className="size-16">
          {leader?.pictureUrl && (
            <AvatarImage src={leader.pictureUrl} alt={leaderName} />
          )}
          <AvatarFallback className="bg-muted text-foreground text-sm font-semibold">
            {initials}
          </AvatarFallback>
        </Avatar>

        <div className="min-w-0 w-full space-y-0.5">
          <p className="text-sm font-semibold text-foreground truncate">
            {bacenta.name} Bacenta
          </p>
          {leaderName && (
            <p className="text-xs text-muted-foreground truncate">
              {leaderName}
            </p>
          )}
        </div>

        {children && <div className="w-full">{children}</div>}
      </Body>

      {showContact && (
        <div className="flex gap-2 border-t border-border px-3 py-3">
          {leader?.phoneNumber && (
            <a href={`tel:${leader.phoneNumber}`} className="flex-1">
              <Button
                type="button"
                variant="outline"
                size="sm"
                className="w-full gap-1.5 min-h-11"
              >
                <Phone className="h-3.5 w-3.5" />
                Call
              </Button>
            </a>
          )}
          {leader?.whatsappNumber && (
            <a
              href={`https://wa.me/${leader.whatsappNumber}`}
              className="flex-1"
            >
              <Button
                type="button"
                variant="outline"
                size="sm"
                className="w-full gap-1.5 min-h-11 text-success border-success/30"
              >
                <svg
                  viewBox="0 0 24 24"
                  className="h-3.5 w-3.5 fill-current"
                  xmlns="http://www.w3.org/2000/svg"
                  aria-hidden="true"
                >
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
                </svg>
                WhatsApp
              </Button>
            </a>
          )}
        </div>
      )}
    </div>
  )
}

export default BacentaArrivalsCard
