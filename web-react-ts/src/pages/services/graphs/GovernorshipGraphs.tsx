...
export const GovernorshipGraphs = () => {
  ...
  const { data, loading, error } = useQuery(GOVERNORSHIP_GRAPHS, {
    variables: { id: governorshipId, limit: TREND_HISTORY_WEEKS, skip: 0 },
  })
  ...
  const jointServiceData = useMemo(
    () =>
      getServiceGraphData(governorship, 'services', TREND_HISTORY_WEEKS) || [],
    [governorship]
  )
  ...
  // Drop bussing aggregate records older than last year to guard against stale dormant data.
  ...
  // When the Joint Service tab is active the window reflects individual joint records,
  // not the network-wide rollup — pin attendance/income stats to the service aggregate.
