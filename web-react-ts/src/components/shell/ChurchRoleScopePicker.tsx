  ...
  return (
    <div className="px-1">
      ...
      <Select value={selectedScopeKey} onValueChange={setSelectedScopeKey}>
        <SelectTrigger
          className="h-9 w-full border-sidebar-border bg-sidebar-accent/50 text-sidebar-foreground data-placeholder:text-sidebar-foreground/60"
        ...
        <SelectContent align="start" className="max-h-80">
          {roleChurchOptions.map((option) => (
            <SelectItem key={option.key} value={option.key}>
              {option.churchName} · {option.roleDisplayName}
            </SelectItem>
          ))}
        </SelectContent>
      </Select>
