...
const MemberDisplayCard = (props: MemberDisplayCardProps) => {
  ...
  switch (member.__typename) {
    case 'Member':
      name =
        (member as CardMember)?.nameWithTitle ||
        (member as CardMember).firstName + ' ' + (member as CardMember).lastName
      details = [
        (member as CardMember).bacenta
          ? (member as CardMember).bacenta!.name + ' Bacenta'
          : '',
        (member as CardMember).basonta
          ? (member as CardMember).basonta!.name
          : '',
      ]
      break
    default:
  ...
  const initials =
    member.__typename === 'Member'
      ? `${(member as CardMember).firstName?.[0] ?? ''}${
          (member as CardMember).lastName?.[0] ?? ''
        }`
